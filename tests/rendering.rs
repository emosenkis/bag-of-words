use word_deck::{
    render_html, render_html_for_page, render_text, render_typst_for_page_with_layout,
};
...
fn typst_export_escapes_quotes_and_uses_cutout_layout() {
    let typst = render_typst_for_page_with_layout(
    ...
    assert!(typst.contains("#set table(stroke: none, inset: (x: 2mm, y: 3mm)"));
    assert!(typst.contains("let cell-inset = 4mm"));
    assert!(typst.contains("let row-height = measure(box(inset: (y: 3mm))[Ag]).height"));
